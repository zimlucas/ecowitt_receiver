...
// --------------------------------------------------------------------------
export async function postWindy(raw) {
  ...
  const obs = { dateutc: nowUtcString() };
...
// --------------------------------------------------------------------------
export async function postWunderground(raw) {
...
// --------------------------------------------------------------------------
export async function postWindguru(raw) {
  const tempC = fahrenheitToCelsius(raw.temp_f);
...
export async function postAll(raw) {
  const [windy, wunderground, windguru] = await Promise.all([
    postWindy(raw),
    postWunderground(raw),
    postWindguru(raw),
  ]);
